    ...
    public function alink($info,$internal=0){
        ...
        switch($info['status']){
            ...
            case 'sol':
                ...
                $i = $info['nCallback'];
                $this->ez['links'][$i] = ['x'=>$info['x'],'y'=>$info['y'],'angle'=>$info['angle'],'descender'=>$info['descender'],'height'=>$info['height'],'url'=>$info['p']];
                if ($internal==0){
                    $this->saveState();
                    $this->setColor(0,0,1);
    ...
    public function uline($info){
        ...
        switch($info['status']){
            ...
            case 'sol':
                ...
                $i = $info['nCallback'];
                $this->ez['links'][$i] = ['x'=>$info['x'],'y'=>$info['y'],'angle'=>$info['angle'],'descender'=>$info['descender'],'height'=>$info['height']];
                $this->saveState();
                $thick = $info['height']*$lineFactor;
                $this->setLineStyle($thick);
                break;
    ...
    public function color($info)
    {
        ...
        switch ($info['status']) {
            ...
            case 'sol':
                ...
                $i = $info['nCallback'];
                $this->ez['links'][$i] = ['x' => $info['x'], 'y' => $info['y'], 'angle' => $info['angle'], 'descender' => $info['descender'], 'height' => $info['height'], 'color' => $info['p']];
                $this->saveState();
